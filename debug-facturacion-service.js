...
const facturacionService = require('./src/modules/facturacion/facturacion.service');

async function debug() {
    console.log('--- Iniciando Depuración de Facturación (SERVICE LEVEL) ---');
    let pool;
    try {
        ...
        console.log('[ACTION] Intentando FacturacionService.createFactura...');
        const result = await facturacionService.createFactura(dummyFactura, uid, eid);
        console.log(`✅ EXITO: Factura creada con Nro: ${result.nro_factura}`);

    } catch (err) {
        console.error('❌ ERROR DURANTE LA EMISIÓN (SERVICE):');
