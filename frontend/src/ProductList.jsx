                  ...
                  <div className="w-10 h-10 rounded-xl bg-surface-50 border border-slate-100 flex-shrink-0 flex items-center justify-center overflow-hidden">
                      {(() => {
                          ...
                          if (p.image_url && typeof p.image_url === 'string') {
                              try {
                                  let parsed = p.image_url;
                                  if (p.image_url.startsWith('[')) parsed = JSON.parse(p.image_url)[0];
                                  if (typeof parsed === 'string') finalImageStr = parsed.replace(/\\/g, '/');
                              } catch(e) {}
                          }
                          const backendRoot = (import.meta.env.VITE_API_URL || '').replace(/\/api\/v\d+\/?$/, '');
                          const imageUrl = finalImageStr ? (finalImageStr.startsWith('http') ? finalImageStr : `${backendRoot}${finalImageStr.startsWith('/') ? '' : '/'}${finalImageStr}`) : null;
