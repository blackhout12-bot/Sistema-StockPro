...
(async () => {
    try {
        ...
        const prodRes = await axios.get('http://localhost:5000/api/v1/productos?page=1&limit=20', {
            headers: { Authorization: `Bearer ${token}` }
        });
        console.log("Productos devueltos:", prodRes.data.data.length);

        console.log("Intentando listar rubros schemas...");
        const schRes = await axios.get('http://localhost:5000/api/v1/productos/categorias/esquemas', {
            headers: { Authorization: `Bearer ${token}` }
        });
        console.log("Schemas devueltos:", schRes.data.length);
